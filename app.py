...
# CORRECCIÓN CLAVE: Quitamos 'eventlet' para que sea compatible con gthread en Render
...
def descargar_certificado(nombre):
    ruta_pdf = f"certificados_emitidos/Certificado_{nombre.replace(' ', '_')}.pdf"
...
def cambiar_modulo(data):
    ...
    votos_actuales = {"A": 0, "B": 0, "C": 0}
    emit('actualizar_grafico', votos_actuales, broadcast=True)
    # Le avisamos a los celulares que hay una nueva pregunta para que se desbloqueen
...
def calcular_premios():
    ...
    for alumno in puntajes_alumnos.keys():
        ...
        premios_asignados[alumno] = titulo
        crear_certificado_premio(alumno, titulo)
